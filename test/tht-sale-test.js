...
import {
    increaseTimeTo,
    duration,
    default as increaseTime
} from '../node_modules/zeppelin-solidity/test/helpers/increaseTime';
...
contract('TokenSale', (accounts) => {
    let owner, client, client2, token, tokensale, startTime, endTime, afterEndTime, timePhase2;
    let testRate, minEthInvestmentPhase1, minEthInvestment, maxEthInvestment, softCap, hardCap;
    let wallet, walletCoreTeam, walletPlatform, walletBountyAndAdvisors;
    let Token = THToken;
    let stages;
    let multisigWallet;
    ...
    before(async() => {
        ...
        walletPlatform = web3.eth.accounts[5];
        walletBountyAndAdvisors = web3.eth.accounts[6];
    });
    ...
    beforeEach(async() => {
        ...
        timePhase2 = startTime + duration.days(4);
        // wallet = (await MultiSigWallet.new(config.MULTISIG_WALLET_TEAM_OWNERS, config.MULTISIG_WALLET_TEAM_REQ_SIG)).address;
        tokensale = await TokenSale.new(startTime, wallet, walletCoreTeam, walletPlatform, walletBountyAndAdvisors);
        ...
        minEthInvestmentPhase1 = await tokensale.MIN_INVESTMENT_PHASE1();
        maxEthInvestment = ether(3000); // TODO: Testing only, determined via whitelisting
    });
    ...
    describe('#token contract tests', async() => {
        ...
        it("should forbid transfer and transferFrom if soft cap not reached and finishCrowdsale called", async() => {
            ...
            shouldHaveException(token.transferFrom(client, owner, 10));
            shouldHaveException(token.transfer(client, 10));
        });
    ...
    describe('#beforeSale', async() => {
        ...
        it('can not buy if not initialized', async() => {
            await assertRevert(tokensale.sendTransaction({amount: ether(10)}));
        });
    ...
    describe('#ongoing sale', async() => {
        ...
        it("should reject buying less than minEthInvestmentPhase1 during phase 1", async() => {
            let veryLowBuyIn = minEthInvestmentPhase1.minus(1);
        ...
        it("should accept buying at least minEthInvestmentPhase1 during phase 1", async() => {
            let buyIn = minEthInvestmentPhase1;
            await tokensale.addWhitelist(client, maxEthInvestment, {from: owner});
        ...
        it("should reject buying less than minEthInvestment during phase2+", async() => {
            let veryLowBuyIn = 0.5 * minEthInvestment;
            await tokensale.addWhitelist(client, maxEthInvestment, {from: owner});

            await increaseTimeTo(timePhase2);
            await assertRevert(tokensale.buyTokens(client, {from: client, value: veryLowBuyIn}));
        });

        it("should accept buying minEthInvestment during phase2+", async() => {
            let buyIn = minEthInvestment;
            await tokensale.addWhitelist(client, maxEthInvestment, {from: owner});
        ...
        it('should accept between min and max in Stage 1, respect rate, partial buys and stage cap', async() => {
            ...
            await assertRevert(tokensale.buyTokens(client, {
                from: client,
                value: minEthInvestmentPhase1.minus(1),
                gasPrice: 0
            }));
            (await tokensale.balanceOf(client)).should.be.bignumber.equal(new BigNumber(0));

            // Client 1 - min purchase
            let buyInMin = minEthInvestmentPhase1;
            ...
            let buyInJustAboveMin = minEthInvestmentPhase1.plus(3);
        ...
        it("should NOT allow withdraw when !softCapReached", async() => {
            ...
            await tokensale.buyTokens(client, {from: client, value: ether(20)});
            (await tokensale.softCapReached()).should.be.equal(false);
    ...
    describe('#after sale tests', async() => {
        it('should allow finalizing crowdsale if endTime reached', async() => {
            ...
            shouldHaveException(token.mintingFinished()); // token contract destroyed
        });
        ...
        it("should refund eth to backers if soft cap not reached after sale", async() => {
            ...
            (await tokensale.hasEnded()).should.be.equal(true);
            (await tokensale.softCapReached()).should.be.equal(false);
            (await tokensale.balanceOf(client)).should.be.bignumber.equal(ether(10).times(5040));

            await tokensale.finishCrowdsale({from: owner});
            await increaseTime(duration.weeks(1));

            await assertRevert(tokensale.balanceOf(client));
        ...
        it("should not allow randoms to call endMinting", async() => {
            await assertRevert(token.endMinting(false, {from: client}));
            ...
            // token contract should be destroyed
            await assertRevert(tokensale.balanceOf(client));
        });
        ...
        it("should create Bounty and Platform tokens if softcap reached", async() => {
            ...
            (await tokensale.balanceOf(walletBountyAndAdvisors)).should.be.bignumber.equal(bountyAllocation);
            (await tokensale.balanceOf(walletPlatform)).should.be.bignumber.equal(platformAllocation);
        });
        ...
        it("should unlock advisor tokens 180 days after start", async() => {
            ...
            let walletPreWithdraw = await token.balanceOf(walletBountyAndAdvisors);

            await tokensale.withdrawAdvisorTokens({from: owner});
            (await token.balanceOf(walletBountyAndAdvisors)).should.be.bignumber.equal(walletPreWithdraw);
            await increaseTimeTo(startTime + duration.days(181));

            let vestedAdvisorAlloc = await tokensale.vestedAdvisors();
            await tokensale.withdrawAdvisorTokens({from: owner});
            (await token.balanceOf(walletBountyAndAdvisors)).should.be.bignumber.equal(walletPreWithdraw.plus(vestedAdvisorAlloc));
